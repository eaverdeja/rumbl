defmodule RumblWeb.VideoControllerTest do
  ...
  test "authorizes actions against access by other users", %{conn: conn} do
    ...
    non_owner = user_fixture(name: "sneaky")
    conn = assign(conn, :current_user, non_owner)

    Enum.each(
      [
        fn -> get(conn, Routes.video_path(conn, :show, video)) end,
        fn -> get(conn, Routes.video_path(conn, :edit, video)) end,
        fn -> get(conn, Routes.video_path(conn, :update, video, video: @create_attrs)) end,
        fn -> get(conn, Routes.video_path(conn, :delete, video)) end
      ],
      fn request ->
        assert_error_sent(:not_found, request)
      end
    )
  end

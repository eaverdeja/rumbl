defmodule Rumbl.MultimediaTest do
  ...
  describe "videos" do
    ...
    test "list_user_videos/1 returns all of the user's videos" do
      owner1 = user_fixture()
      owner2 = user_fixture()
      %Video{id: id1} = video_fixture(owner1)
      %Video{id: id2} = video_fixture(owner2)
      assert [%Video{id: ^id1}] = Multimedia.list_user_videos(owner1)
      assert [%Video{id: ^id2}] = Multimedia.list_user_videos(owner2)
    end
    ...
    test "get_user_video!/2 returns the video with the given id and user" do
      owner1 = user_fixture()
      owner2 = user_fixture()
      %Video{id: id1} = video_fixture(owner1)
      %Video{id: id2} = video_fixture(owner2)
      assert %Video{id: ^id1} = Multimedia.get_user_video!(owner1, id1)
      assert %Video{id: ^id2} = Multimedia.get_user_video!(owner2, id2)
      assert_raise Ecto.NoResultsError, fn -> Multimedia.get_user_video!(owner1, id2) end
    end
